...
$(function() {
	...
	$('.switch-systems').click(function () {
		...
		$.cookie('share-upms-systemid', systemid);
		$.cookie('share-upms-systemname', systemname);
		$.cookie('share-upms-systemtitle', systemtitle);
	});
	// 显示cookie菜单
	var systemid = $.cookie('share-upms-systemid') || 1;
	var systemname = $.cookie('share-upms-systemname') || 'share-upms-server';
	var systemtitle = $.cookie('share-upms-systemtitle') || '权限管理系统';
	$('.system_menus').hide(0, function () {
		$('.system_' + systemid).show();
	});
	$('body').attr('id', systemname);
